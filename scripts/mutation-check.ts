...
const MUTATIONS: Mutation[] = [
  ...
  { name: "a text message pretending to be the network is believed", file: "src/bridge.ts", find: "  if (!fromTheNetwork(msg.from, senderIds)) {", replace: "  if (false) {", suite: "tests/bridge.test.ts" },
  { name: "any sender name believed on every network", file: "src/bridge.ts", find: "  if (allowed.length === 0) return false;\n  return allowed.includes(plain(from));", replace: "  return true;", suite: "tests/bridge.test.ts" },
  { name: "a code that changes the phone itself is sent to a SIM", file: "src/sendingphone.ts", find: "  if (!looksLikeTopUpCode(code)) {", replace: "  if (false) {", suite: "tests/sendingphone.test.ts" },
  { name: "a PIN typed into a network code is accepted", file: "src/settings.ts", find: "    if (r.value !== \"\" && !r.value.includes(\"{pin}\") && /\\d{4,}/.test(r.value)) {", replace: "    if (false) {", suite: "tests/settings.test.ts" },
  { name: "an agent top-up reference says who paid and when", file: "src/agent/portal.ts", find: "const reference = `AT-${randomBytes(6).toString(\"base64url\").toUpperCase().replace(/[^A-Z0-9]/g, \"\")}`;", replace: "const reference = `AT-${s.agent.id}-${Date.now().toString(36).toUpperCase()}`;", suite: "tests/agents.test.ts" },
  ...
  { name: "audit actor ignored", file: "migrations/0011_audit_secrets.sql", find: "actor  text := coalesce(nullif(current_setting('app.actor', true), ''), 'system');", replace: "actor  text := 'system';", suite: "tests/settings.test.ts" },
  ...
  { name: "custom pattern ignored", file: "src/bridge.ts", find: ": parseNetworkMessage(text, patterns[device.network_code]);", replace: ": parseNetworkMessage(text, \"\");", suite: "tests/bridge.test.ts" },
  { name: "unreadable airtime message dropped silently", file: "src/bridge.ts", find: "return finish(looksLikeValue(text) ? \"unparsed\" : \"ignored\", null, `${parsed.problem} ${data.problem}`);", replace: "return finish(\"ignored\", null, `${parsed.problem} ${data.problem}`);", suite: "tests/bridge.test.ts" },
  ...
  { name: "sender numbers shown in full on the status page", file: "src/public/pages.ts", find: "return `${number.slice(0, 4)} **** ${number.slice(-2)}`;", replace: "return number;", suite: "tests/public.test.ts" },
  ...
  { name: "data message read as one naira of airtime (both defences off)", file: "src/bridge.ts", find: "const mentionsData = /\\d\\s*[GM]B/i.test(text);", replace: "const mentionsData = false;", also: [{ find: "(?!\\s*[GM]B)", replace: "" }], suite: "tests/data.test.ts" },
  ...
  { name: "network's text message not used to settle a command", file: "src/bridge.ts", find: "  const settled = await confirmFromMessage(db, device.id, text);", replace: "  const settled = undefined as Awaited<ReturnType<typeof confirmFromMessage>>;", suite: "tests/sendingphone.test.ts" },
